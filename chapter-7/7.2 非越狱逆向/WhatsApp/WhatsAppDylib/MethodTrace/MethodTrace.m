...
+(void)addClassTrace:(NSString *)className methodList:(NSArray *)methodList{
    ...
    if(targetClass != nil){
        [ANYMethodLog logMethodWithClass:NSClassFromString(className) condition:^BOOL(SEL sel) {
            ...
            NSString* outputString = [NSString stringWithFormat:@"%@[%@ %@]", deepString , target, selectorString];
            NSLog(@"%@",outputString);
            ...
            NSString* outputString = [NSString stringWithFormat:@"%@ret:%@", deepString, retValue];
            NSLog(@"%@", outputString);
            printf("%s\n", [outputString UTF8String]);
        }];
    }else{
